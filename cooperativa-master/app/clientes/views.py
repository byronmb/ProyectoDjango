...
def principalCuenta(request):
    ...
    if usuario.has_perm('modelo.view_cliente'):
        ...
        context = {
            ...
            'clientes': listaClientes,
            'title': "Cuentas",
            'mensaje': "Lista de Cuentas"
        }
        return render(request, 'clientes/home_cuenta.html')
    else:
        messages.warning(request, 'No Permitido')
        return render(request, 'login/403.html')
...
def crear(request):
    ...
    if usuario.has_perm('modelo.add_cliente'):
        ...
        messages.warning(request, 'No Permitidoc')
        return render(request, 'login/403.html')
...
def buscar(request):
    ...
    if usuario.has_perm('modelo.view_cliente'):
        ...
        if lista.exists() == True:
            ...
            messages.warning(request, 'No existe el cliente')
            context = {
...
def buscarCuenta(request):
    ...
    if usuario.has_perm('modelo.view_cliente'):
        dni = request.GET['txt_buscar']
        try:
            lista = Cuenta.objects.filter(cliente=Cliente.objects.get(cedula=dni))
            context = {
...
def buscarTrans(request):
    ...
    if usuario.has_perm('modelo.view_cliente'):
        numero = request.GET['numero']
        transaccion = Cuenta.objects.all()
        arreglo =""
        for c in transaccion:
            arreglo = arreglo + c.numero + ','
        context = {
            'arreglo': arreglo,
